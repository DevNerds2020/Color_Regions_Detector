...
def process_image(image_path, include_colors=["red", "green", "blue"]):
    # Load the image
    ...
    total_pixels = sum(
        count for color, count in color_pixel_counts.items() if color in include_colors
    )
    ...
    for color, count in color_pixel_counts.items():
        ...
        hex_value = hex_values.get(
            color, "#000000"
        )  # Get hex value for color, default to black if not found
...
def analyze_image():
    ...
    include_colors = request.form.get("includeColors")
    if file.filename == "":
    ...
    if file:
        ...
        file_path = filename
        file.save(file_path)
        color_percentages = process_image(file_path, include_colors)
